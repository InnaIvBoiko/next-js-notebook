'use client';
// Reusable Client island for each sub-route's localized header.
// Same pattern as Lesson 4: Server demo pages render <DemoHeader which="..." />
// to get a multilingual title without becoming Client components themselves.

import Link from 'next/link';
import { content } from '../_lib/content';
import { useLessonLang } from './lang-provider';

type DemoKey = 'default' | 'parallel' | 'dynamic';

const ACCENT: Record<DemoKey, string> = {
    default: 'border-sky-500/30 bg-sky-500/10 text-sky-300',
    parallel: 'border-violet-500/30 bg-violet-500/10 text-violet-300',
    dynamic: 'border-amber-500/30 bg-amber-500/10 text-amber-300',
};
