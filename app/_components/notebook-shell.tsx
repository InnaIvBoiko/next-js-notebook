...
function StatusBadge({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-medium uppercase tracking-wide ${styles}`}
    >
      {status === "done" && "✓"}
      {status === "next" && "→"}
      {status === "locked" && "·"}
              ...
              <ul className="mt-4 space-y-2">
                {m.lessons.map((lesson) => {
                  ...
                  return (
                    <li key={lesson.slug}>
                      {isInteractive ? (
                        <Link href={`/lessons/${lesson.slug}`}>{content}</Link>
                      ) : (
                        content
                      )}
                    </li>
